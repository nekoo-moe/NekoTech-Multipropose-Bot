...
1. Clone mã nguồn về máy hoặc giải nén file ZIP:
   ```bash
   git clone https://github.com/nekoo-moe/NekoTech-Multipropose-Bot/
   cd NekoTech-Multipropose-Bot
   ```
...
### Bước 3: Thiết Lập Cấu Hình (`config/config.yml`)
1. Mở file `config.yml` nằm ở thư mục /config.
