...
exports.default = new Event_1.Event("interactionCreate", (e) =>
  tslib_1.__awaiter(void 0, void 0, void 0, function* () {
    if (e.isChatInputCommand()) {
      ...
      if (!t) return;
      if (t.guildId && t.guildId !== e.guild.id) return;
      const cmdConfig = __1.client.commandsConfig[t.permission] || {
        Enabled: true,
      ...
      if (
        !isOwner &&
        !isAdmin &&
        !e.member.roles.cache.some(
          (role) => r.includes(role.id) || r.includes(role.name),
        ) &&
        !r.includes(e.member.id) &&
        !r.includes("@everyone")
      )
        return e.reply({
          embeds: [
      ...
      try {
        const r = `${e.user.id}-${t.name}`,
          i = __1.client.cooldowns.get(r) || 0,
          o = s ? (0, ms_1.default)(s) : 0;
        if (i - Date.now() > 0)
          return e.reply({
            embeds: [
              (0, replaceAll_1.default)(
                __1.client.messages.Embeds.UserInCooldownEmbed,
                {
                  "{user-tag}": e.user.tag,
                  "{user-avatar}": e.user.displayAvatarURL(),
                  "{time}": `${Math.floor(i / 1e3)}`,
                },
              ),
            ],
          });
        (yield t.run({ client: __1.client, interaction: e }),
          o > 0 &&
            (__1.client.cooldowns.set(r, Date.now() + o),
            setTimeout(() => {
              __1.client.cooldowns.delete(r);
            }, o)));
      } catch (t) {
        if ("exclude" === (null == t ? void 0 : t.message)) return;
        (console.error(t),
          e
            .reply({
              embeds: [
