...
function generateRandomReels(e) {
  const n = [];
  for (let o = 0; o < e; o++) {
    const e = symbols[Math.floor(Math.random() * symbols.length)];
    n.push(e);
  }
  return n;
}
function checkWinningCombinations(e) {
  for (const n of winningCombinations) {
    const { symbols: o, payout: t } = n;
    if (e[0] === o[0] && e[1] === o[1] && e[2] === o[2]) return { payout: t };
  }
  return null;
}
function generateSlotAnimation(e, n) {
  const o = [];
  for (let t = 0; t < n; t++) {
    let s = "";
    for (let o = 0; o < 3; o++) {
