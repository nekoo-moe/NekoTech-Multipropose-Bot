...
function randomNumbers() {
  ...
  for (let t = 0; t < 25; t++) {
    ...
    (e.includes(t) && (t = Math.floor(1e3 * Math.random() + 1)), e.push(t));
  }
  return e;
}
function getButtons(e, t = [], s = !1, o) {
  const n = [];
  let r = new discord_js_1.ActionRowBuilder();
  for (const d of e) {
    const e = new discord_js_1.ButtonBuilder()
      .setCustomId(d.toString())
      .setLabel(d.toString())
      .setDisabled(!!t.includes(d) || s)
      .setStyle(
        o === d
    ...
    (r.addComponents(e),
      5 === r.components.length &&
        (n.push(r), (r = new discord_js_1.ActionRowBuilder())));
  }
  return (r.components.length > 0 && n.push(r), n);
}
